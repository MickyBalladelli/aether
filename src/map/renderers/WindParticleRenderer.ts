...
const WIND_MAX_ZOOM_SCALE = 3.2
const WIND_MAX_STREAK = 22
...
export class WindParticleRenderer extends ParticleModeRenderer {
  ...
  draw(samples: ProjectedSample[], deltaTime: number) {
    ...
    const zoomScale = Math.min(
      ...
      Math.max(
        0.8,
        2 ** ((this.map.getZoom() - WIND_REFERENCE_ZOOM) * 0.35)
      )
    )
    ...
    for (let index = 0; index < activeCount; index += 1) {
      ...
      const streak = Math.min(WIND_MAX_STREAK, Math.max(tail, movement * 1.1))
